...
public class SearchAndSort
{
     ...
     * Note: This file contains only methods and will not run in the code editor
     *
     * Preconditions: rt > lt, arr[], lt, rt, and x all contain only integer values.
     *
     */
    public static int binarySearch(int arr[], int lt, int rt, int x)
    {
        ...
        {
            int mid = lt + (rt - lt) / 2;
            // Checks if the search has completed
            if (arr[mid] == x)
                return mid;
            // If the value at midpoint is greater, it runs again
            if (arr[mid] > x)
    ...
    public static int itBinSearch(int arr[], int lt, int rt, int x)
    {
        if (rt >= lt)
        {
            int mid = lt + (rt - lt) / 2;
